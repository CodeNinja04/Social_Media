...
export const Mutation = {
    postCreate: async (_: any, { post }: any, { prisma }: Context):Promise<PostPayloadType> => {

        const{title,content} =post;
        if(!title || !content){
            return{
            userErrors:[{ message:"provide title and content"}],
            post:null
        }}
    ...
    postUpdate: async (_: any, { post, postId }: { postId: string, post: PostArgs["post"] }, { prisma }: Context): Promise<PostPayloadType> => {
        ...
        return {
            ...
            post:prisma.post.update({
              data:{...payloadToUpdate},where : { id:Number(postId)}
            })
        }
